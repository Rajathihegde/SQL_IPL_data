...
select * from IPL_Bidding_Details;
select bidder_id,sum(bid_status like '%won%') from IPL_bidding_details group by bidder_id;
select count(bid_status) from IPL_bidding_details ;


select bidder_id , sum(bid_status like '%won%')/count(bid_status)*100 as percent_win from IPL_Bidding_Details group by bidder_id order by percent_win desc;
...
select st.Stadium_name, st.city,count(st.stadium_id)as Num_of_match from ipl_stadium st join ipl_match_schedule ms 
on st.stadium_id=ms.stadium_id group by st.stadium_id;
...
select st.stadium_id,st.stadium_name,sum(im.toss_winner=im.match_winner)/count(ms.match_id)*100 as `toss_match_winner` from ipl_match im  
join ipl_match_schedule ms on im.match_id=ms.match_id 
join ipl_stadium st on ms.stadium_id=st.stadium_id 
...
select bd.bid_team,t.team_name,count(bid_team) as total_bids from IPL_Bidding_Details bd join IPL_Team  t 
on t.team_id=bd.bid_team group by t.team_name;
...
select TS.team_id,t.team_name,sum(matches_played) as matches_played,sum(matches_won) matches_won,sum(matches_lost) as matches_lost 
from ipl_team_standings TS 
join ipl_team t on TS.team_id=t.team_id group by t.team_id;
...
select player_id,player_name,team_name, player_role,player_id from
 ( select t.team_name as team_name ,p.player_id as player_id,p.player_name as player_name  ,tp.player_role as player_role  from IPL_team t 
 join IPL_Team_players tp on t.team_id=tp.team_id join IPL_Player p on tp.player_id=p.player_id ) player_details where team_name ='Mumbai Indians' and player_role='Bowler';
...
select team_id,team_name, player_role,count(player_role) as count_of_All_rounder from
 ( select t.team_id as team_id ,t.team_name as team_name ,p.player_id as player_id,p.player_name 
 as player_name  ,tp.player_role as player_role   from IPL_team t 
 join IPL_Team_players tp on t.team_id=tp.team_id join IPL_Player p on tp.player_id=p.player_id ) player_details 
 where player_role ='All-Rounder' group by team_name having count_of_all_rounder>4;